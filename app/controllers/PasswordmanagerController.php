		...
		if(!empty($this->post->email)){
			...
			if(!empty($user)){
				...
				if($mailer->send_mail($email, $mailtitle, $mailbody) == true){
					...
					$msg = "Error haruka email Adrees. favor contac ba administrator sistema atu hatene informasaun Klean Obrigadu";
					$this->render_view("errors/error_general.php", $msg, "info_layout.php");
				}
			}
			else{
				$this->set_page_error("Email adrees Refere Seidauk Rejistu ba Sistema Obrigadu");
				$this->render_view("passwordmanager/index.php", null, "info_layout.php");
			}
		}
		else{
